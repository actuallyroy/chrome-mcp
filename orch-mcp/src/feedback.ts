...
// authored under their account; falls back to the shared-token endpoint when
// `gh` isn't installed/authenticated. Mirrors android-mcp/src/feedback.ts and
// chrome-mcp/src/feedback.ts — same repo, same shape, just a different
// `product` label.
...
const REPO =
  process.env.ORCH_MCP_FEEDBACK_REPO ||
  process.env.CHROME_MCP_FEEDBACK_REPO ||
...
async function fileViaGh(input: FileFeedbackInput): Promise<FileFeedbackResult> {
...
export async function fileFeedback(input: FileFeedbackInput): Promise<FileFeedbackResult> {
  if (process.env.ORCH_MCP_FEEDBACK_NO_GH || process.env.CHROME_MCP_FEEDBACK_NO_GH) {
    return fileViaEndpoint(input);
  }
  const login = await ghAuthenticated();
  if (login) {
    try {
      return await fileViaGh(input);
    } catch (e) {
