#!/usr/bin/env node
// orch-mcp loader. Downloads the latest bundle from the Vercel deployment,
// verifies its SHA-256, caches it, and runs it as the MCP server over stdio.
//
// Zero npm deps — only Node ≥18 stdlib.
//
// Env:
//   ORCH_MCP_ENDPOINT     override Vercel origin (default: chrome-mcp.actuallyroy.com)
//   ORCH_MCP_PIN_VERSION  pin to a specific version; skips update check
//   ORCH_MCP_SKIP_UPDATE  truthy → always use cached bundle, don't hit network
//   ORCH_MCP_CACHE_DIR    override cache dir (default: ~/.orch-mcp)
...
const ENDPOINT =
  (process.env.ORCH_MCP_ENDPOINT || "https://chrome-mcp.actuallyroy.com").replace(/\/$/, "");
const CACHE_DIR = process.env.ORCH_MCP_CACHE_DIR || join(homedir(), ".orch-mcp");
const STATE_FILE = join(CACHE_DIR, "state.json");
const PIN = process.env.ORCH_MCP_PIN_VERSION || null;
const SKIP_UPDATE = /^(1|true|yes)$/i.test(process.env.ORCH_MCP_SKIP_UPDATE || "");

function log(...args) { console.error("[orch-mcp]", ...args); }
...
async function fetchJson(url) {
  const res = await fetch(url, { headers: { "user-agent": "orch-mcp-loader/1" } });
...
async function resolveBundle() {
  ...
  let manifest;
  try {
    manifest = await fetchJson(`${ENDPOINT}/orch/bundle/manifest.json`);
  } catch (err) {
    ...
    throw new Error(`orch-mcp: cannot reach ${ENDPOINT} and no cached bundle. ${err.message}`);
  }
  ...
  log(`downloading orch-mcp v${manifest.version} from ${bundleUrl}`);
  const bytes = await fetchBytes(bundleUrl);
  const actual = sha256Hex(bytes);
  if (actual !== manifest.sha256) {
    throw new Error(`orch-mcp: SHA-256 mismatch. Expected ${manifest.sha256}, got ${actual}.`);
  }
...
async function main() {
  try {
    ...
    if (isFirstRun) {
      log("");
      log("orch-mcp is licensed under GNU GPL v3 (copyleft). By using it you");
